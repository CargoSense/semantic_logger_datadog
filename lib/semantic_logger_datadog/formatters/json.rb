...
module SemanticLoggerDatadog
  module Formatters
    class Json < SemanticLogger::Formatters::Raw
      # @param time_format [Symbol]
      # @param time_key [Symbol]
      #
      # @see https://github.com/reidmorrison/semantic_logger/blob/v4.17.0/lib/semantic_logger/formatters/raw.rb
      def initialize(time_format: :iso_8601, time_key: :timestamp, **args)
        super
      end

      # @param log [SemanticLogger::Log]
      # @param logger [SemanticLogger::Appender::IO]
      #
      # @return [String]
      def call(log, logger)
        super
